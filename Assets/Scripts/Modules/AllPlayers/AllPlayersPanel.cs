...
{
    public class AllPlayersPanel : BaseModulePanel
    {
        ...
        public override void Initialize(UIManager manager, GameObject container)
        {
            ...
            GameObject scrollView = uiManager.CreateScrollViewHelper(panelContainer.transform, "MarketScroll", out listContent);
        ...
        public override void Refresh()
        {
            ...
            // Filter players who are favorited or contacted!
            List<Player> watchList = DatabaseManager.Instance.Players.FindAll(p => p.IsFavorite || p.IsContacted);

            // Sort watchList dynamically based on chosen sort parameters
            watchList.Sort((a, b) => {
                int compare = 0;
            ...
            {
                GameObject emptyRow = uiManager.CreatePanelHelper(listContent, "EmptyWatchlist", new Color(0.15f, 0.17f, 0.22f, 0.4f));
                LayoutElement le = emptyRow.AddComponent<LayoutElement>();
                le.minHeight = 350f;
                le.preferredHeight = 350f;

                Text info = CreateText(emptyRow.transform, "EmptyInfo", "<b>🔍 İZLEME LİSTESİ BOŞ</b>\n\nFavoriye eklediğiniz veya temasa geçtiğiniz oyuncular burada listelenir.\nOyuncuları gözlemci raporlarından favorileyebilirsiniz.", 48, Color.white, TextAnchor.MiddleCenter);
                SetRectTransform(info, Vector2.zero, Vector2.one, new Vector2(30f, 10f), new Vector2(-30f, -10f));
            }
            else
            {
                foreach (var p in watchList)
                {
                    CreateWatchRow(listContent, p);
                }
            }
        }

        private void CreateWatchRow(Transform parent, Player p)
        {
            GameObject row = uiManager.CreatePanelHelper(parent, "WatchRow_" + p.Id, new Color(0.12f, 0.14f, 0.18f, 0.75f));
            ...
            rowBtn.onClick.AddListener(() => uiManager.ShowPlayerDetails(p, false));
